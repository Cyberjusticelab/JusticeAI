...
class RegexLib:
    ...
    regex_demands = {
        ...
        "tenant_cover_rent": re.compile(r"(demande.+)?(locateur|locatrice)(s)?(.+demande)?.+recouvrement\s(du|de)\sloyer"),
        ...
        "landlord_claim_interest_damage": re.compile(r"(locateur|locatrice)(s)?(.+([\d\s,]+)(\$|\s\$)).+(dommages-intérêts)"),
