...
class RegexLib:
    regex_demands = [
        ...
        ("landlord_demand_lease_modification", [re.compile(r"\[[123]\].+produit une demande en modifications du bail")], re.IGNORECASE),
    ...
    regex_facts = [
        ...
        ("disrespect_previous_judgement", [re.compile(r"\[\d+\].+non-respect d\'une ordonnance émise antérieurement"),
        ...
        ("tenant_owes_rent", [re.compile(r"(.*preuve.+locateur(s)?.+non-paiement.+loyer)?.*(locataire(s)?.+(doi(vent|t))((.+somme\sde)|(total))?.+([\d\s,]+)(\$|\s\$)|locateur.+créance.+\b(\d{1,3}(\s\d{3}|,\d{2})*)+(\$|\s\$).+loyers\simpayés)|(.*paiement.*arriérés.+loyer.+\b(\d{1,3}(\s\d{3}|,\d{2})*))"),
                              ...
                              re.compile(r"\[\d+\].+admet devoir la somme de (\d+(\s|\,)){1,2}\$ à titre de loyer"),
                              re.compile(r"\[\d+\].+une reconnaissance de dette")], re.IGNORECASE),
        ("tenant_refuses_retake_apartment", [re.compile(r".*locataire(s)?.+refus(e|ait|aient).+quitter.*")], re.IGNORECASE),
        ("tenant_rent_not_paid_exactly_3_weeks", [re.compile(r"\[\d+\].+locataire(s|) (est|sont) en retard (\w+(\s|\'|,\s)){1,8}(trois|3) semaines")], re.IGNORECASE),
        ...
        ("tenant_rent_late_less_than_3_weeks", [re.compile(r"\[\d+\].+locataire(s|) (n\'est|ne sont) pas en retard (\w+(\s|\'|,\s)){1,8}trois semaines")], re.IGNORECASE),
